...
router.get('/announcements', (req, res) => {
  ...
  const filterByChannel = rows => {
    if (!channel) return rows;
    return rows.filter(ann => ann.target_room?.startsWith('#') && normalizeChannel(ann.target_room) === normalizeChannel(channel));
  };

  if (scope === 'my') return res.json(filterByChannel(getAnnouncementsByOwner(userId)));
  if (scope === 'shared') return res.json(filterByChannel(getSharedAnnouncements()));
  if (scope === 'all' && req.session.user.isAdmin) return res.json(filterByChannel(getAllAnnouncements()));
  return res.json(filterByChannel(getAnnouncementsByOwner(userId)));
});
